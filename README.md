...
- Speaking of deploying, I spent almost a whole day trying to do this as well. Setting up a CI/CD pipeline seemed simple enough with the prospect of using GitHub actions. However, for some reason, every bundle would either fail to build or my tests would fail. I even went on to try to deploy my game onto Heroku manually, without a pipeline, yet to no avail. I decided to take my losses on this one for now, and deploy at a later date.
